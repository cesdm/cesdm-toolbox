...
**Expected output** (entity count may vary with library size):

```text
Validated 42 entities and exported to .../output/minimal_electricity_model
```
...
|--------|----------------|
| Validated model | One electricity domain, one bus, one wind farm, one demand unit |
