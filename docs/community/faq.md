...
See [Validation](../getting-started/validation.md).
...
Attribute groups organize information by modelling perspective (dispatch, topology, power flow, planning, etc.) while keeping all data on the same entity. See [Schemas — Attribute groups](../getting-started/schemas.md#attribute-groups).
...
Run `model.validate()` then `model.validate_for_analysis("<profile>")`. Shipped profiles include `optimal_dispatch` and `power_flow` under `analysis_profiles/`.
