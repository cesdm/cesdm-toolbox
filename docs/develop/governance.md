...
!!! info "TL;DR if you build CESDM study models"
    The current CESDM core schema version is **1.0.0**. Schemas use **semantic versioning** (`MAJOR.MINOR.PATCH`). Exported models record their schema version. You rarely author schemas — but pin that version when sharing models. History: [Release log](../community/changelog.md). Stable vs experimental: [Roadmap](../community/roadmap.md).
...
To ensure long-term compatibility while allowing the data model to evolve, CESDM follows a lightweight schema governance process based on semantic versioning, clearly defined stability levels, and community decision.
...
How a concrete change is classified (patch, minor or major), and how the core schema evolves, is **decided by the community** — through issues, pull requests and discussion, not by a separate RFC board or a fixed set of reviewers. See [Contributing](../community/contributing.md).
...
The versioning table above is a **compatibility rule**, not a closed decision process. There is no separate RFC board and no designated reviewer list. Whether a proposed change is treated as PATCH, MINOR or MAJOR, and whether a family stays experimental or becomes stable, is **established by the community** through issues, pull requests and open discussion ([Contributing](../community/contributing.md)).

When proposing a schema change, contributors should clearly document:

- **What** is changing.
- **Why** the change is needed.
- Whether they consider the change **PATCH**, **MINOR**, or **MAJOR**.
- The expected impact on existing models and tools.

The community then confirms or revises that classification. Keeping the process lightweight encourages contributions while maintaining interoperability and long-term consistency.
...
- **Community decision** on how versioning rules are applied and how the schema evolves.
...
- Version bumps and schema evolution are decided by the community; the rules above only say how compatibility is communicated.
