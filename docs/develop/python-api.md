...
!!! abstract "Before you start"
    - **Prerequisites:** [Build your first model](../get-started/first-model.md), [Schemas](../understand/schemas.md) (attribute groups)
    - **Audience:** advanced modelling in Python, adapter authors, and contributors

The day-to-day interface is schema-aware entity handles (`bus.name = …`, `gen.atNode = bus`). The lower-level `add_attribute` / `add_relation` calls are the same store — see [EAR internals](ear-internals.md).
...
## Why a handle-based API?

CESDM stores models as **Entity–Attribute–Relation ([EAR](../reference/glossary.md#ear))** data. You can write that store with explicit operations:
...
The [Proxy API](../reference/glossary.md#proxy-api) provides a more convenient object-oriented interface to exactly the same data. Groups such as `dispatch` and `topology` come from the schema — see [Schemas — Attribute groups](../understand/schemas.md#attribute-groups).
...
is completely equivalent to the [EAR](../reference/glossary.md#ear) representation above.
...
[Attribute groups](../reference/glossary.md#attribute-group) are declared in schema YAML via `belongsToGroup`. The Proxy API maps them to nested namespaces on each entity — one physical asset, several perspectives:
...
Full definitions and the list of standard groups: [Schemas — Attribute groups](../understand/schemas.md#attribute-groups).

---

## Writing Attributes

The [EAR](../reference/glossary.md#ear) API
...
Using the [Proxy API](../reference/glossary.md#proxy-api):
...
or directly through the [EAR](../reference/glossary.md#ear) API:
...
One of the major advantages of the [Proxy API](../reference/glossary.md#proxy-api) is schema-aware type checking.
...
For typed retrieval of existing entities (`get_entity`, `get_entity_as`), see [Python Typings & Proxies (optional)](python-api.md).

---

## Best Practices

For most applications the [Proxy API](../reference/glossary.md#proxy-api) is recommended because it
...
The [EAR](../reference/glossary.md#ear) API remains the underlying canonical representation and is particularly useful for generic tooling, importers, exporters and schema-independent processing.
...
The **handle-based [Python API](../reference/glossary.md#proxy-api)**

- presents the same model through typed Python objects;
- exposes [attribute groups](../understand/schemas.md#attribute-groups) as nested namespaces;
...
→ [Python typing proxies](python-typing-proxies.md) · [EAR internals](ear-internals.md) · [Build models](../use/build-models.md)
