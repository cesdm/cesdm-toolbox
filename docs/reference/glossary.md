...
For common modelling patterns, see [Build models](../use/build-models.md). For stability tiers, see [Governance](../develop/governance.md).
...
## Attribute
A named property of an [entity](#entity). Attributes describe the characteristics of an object. Together with [relations](#relation), attributes are one of the three building blocks of the [EAR](#ear) model — see [Entities and relationships](../understand/entities-and-relationships.md).

---

## Attribute Group
A logical grouping of schema-defined attributes and relations by modelling perspective, such as `dispatch`, `topology`, or `power_flow`. Declared in schema YAML via `belongsToGroup` — see [Schemas — Attribute groups](../understand/schemas.md#attribute-groups).
...
Every CESDM model consists of [entities](#entity), [attributes](#attribute), and [relations](#relation). See [Entities and relationships](../understand/entities-and-relationships.md).
...
Modellers create **instances** of entity classes; classes are defined in [schemas](#schema) (CESDM core or [extended schemas](../develop/schema-extensions.md)).
...
## Ontology
The catalogue of energy-system **concepts** and how they relate — for example that `GenerationUnit` and `ElectricalBus` are distinct types that `atNode` can connect.

CESDM is built from such a concept graph. It is not introduced as “an energy-system ontology.” See [Entities and relationships](../understand/entities-and-relationships.md#schema-semantics-and-ontology).
...
The Proxy API operates on exactly the same underlying [EAR](#ear) model while improving readability, auto-completion, and type safety. See the [Proxy API guide](../develop/python-api.md).
...
Stability differs by subclass — see [Schema Governance](../develop/governance.md#family-keys).
...
Schemas define the modelling vocabulary; your study model contains [entity instances](#entity). See [Schemas](../understand/schemas.md).
...
## Semantics
The agreed **meaning** of schema fields — for example that `nominal_power_capacity` is rated generation capacity and `atNode` associates a unit with a network node.

Semantics is why the same CESDM object can be reused across tools. See [Entities and relationships](../understand/entities-and-relationships.md#schema-semantics-and-ontology).
...
Individual assets reference technology [entities](#entity) via `hasTechnology` instead of duplicating common information — see [Libraries](../understand/libraries.md).
...
- [Entities and relationships](../understand/entities-and-relationships.md) — entities, attributes, relations, and [schema / semantics / ontology](../understand/entities-and-relationships.md#schema-semantics-and-ontology)
- [Build models](../use/build-models.md) — modelling lifecycle
- [Schema Governance](../develop/governance.md) — versioning and stability tiers
