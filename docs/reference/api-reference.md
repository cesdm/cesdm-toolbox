...
!!! warning "Energy system modellers — start elsewhere"
    For day-to-day model building, use **`cesdm_toolbox`** and the [Proxy API guide](../guides/proxy-api.md). This reference documents the low-level **`ear`** engine package used by integrators and contributors.
...
| `name` | `str` | Package name (defaults to `"ear-model"`). |
