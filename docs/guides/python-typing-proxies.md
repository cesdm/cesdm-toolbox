...
For statically typed access, use `asset_as()`.

---

## Typed Retrieval with `asset_as()`

`asset_as()` tells both CESDM and the type checker which proxy class is expected.

```python
from cesdm.generated_proxies import GenerationUnitProxy

generator = model.asset_as(
    "gen.ch.wind",
    GenerationUnitProxy,
)
```
...
`asset_as()` also verifies the runtime type. It prevents application code from treating an entity as an incompatible proxy class.
...
Use `asset_as()` when retrieving an existing entity and its expected class is known:
