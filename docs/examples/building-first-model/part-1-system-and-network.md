...
The schema defines the permitted [entity classes](../../reference/glossary.md#entity-class), attributes, relations, inheritance, and constraints.

The [Default Library](../../reference/glossary.md#default-library) contributes reusable entity instances such as carriers and technology definitions. The [regions library](../../understand/libraries.md) adds NUTS / country `GeographicalRegion` entities (e.g. `region.country.CH`). Importing libraries does not create project-specific physical assets.

---

## 2. Create the system and electricity Carrier Domain

An entity is created first, then described by attributes and connected through relations. With the [Proxy API](../../reference/glossary.md#proxy-api), you assign them on the handle returned by `add_entity`:
...
!!! abstract "Core EAR alternative"
    ...
    The same steps with explicit [EAR](../../reference/glossary.md#ear) calls:
...
- `ElectricalBus` identifies the [entity class](../../reference/glossary.md#entity-class);
...
← Previous: [Building a CESDM Model](overview.md)  
