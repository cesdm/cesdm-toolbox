...
!!! abstract "Before you start"
    - **Prerequisites:** [Installation](../../get-started/install.md); complete [Build your first model](../../get-started/first-model.md) first
    - **Time:** ~60 minutes across four parts
    - **Audience:** energy system modellers building the full CH + neighbours reference model

!!! note "Start simple"
    Begin with [Build your first model](../../get-started/first-model.md). This four-part tutorial and the [Jupyter notebook](../advanced-reference-notebook.md) cover the same full reference model in more detail.

This tutorial constructs a simplified multi-domain energy system for Switzerland and its neighbouring countries using the [Proxy API](../../reference/glossary.md#proxy-api).
...
Uses typed entity handles (`bus.name = ...`, `gen.atNode = bus`) — the same style as [Build your first model](../../get-started/first-model.md).
...
→ Next: [Part 1 — System and Network](part-1-system-and-network.md)
