...
This regenerates `cesdm/generated_proxies.py`, the type stubs under `typings/`, and `docs/reference/schema_reference.html`. All three are committed artifacts, kept in sync with the schema — a PR that changes a schema file without regenerating these will fail review.
