...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/status-first%20release-2563EB.svg" alt="First release">
</p>
...
| FlexECO (in-house, **separate** package) | `pip install -e /path/to/cesdm-flexeco` — see [Import and export](docs/use/import-export.md) |

External TYNDP / PyPSA sample data (large): `cesdm-download-external-data` → writes under `external_data/`. Already converted CESDM packages (YAML + Frictionless + profiles): `cesdm-download-converted-data` → writes under `converted_data/`. Scenario-Assumptions dashboard from a CESDM YAML: `cesdm-assumptions-dashboard --yaml path/to/model.yaml`. Interactive Load YAML: `cesdm-assumptions-dashboard --serve`.
...
| FlexECO | Separate package [`cesdm-flexeco`](docs/use/import-export.md) |
...
This first release establishes a solid and usable foundation for CESDM, while leaving room for further extensions, new use cases and contributions from the community. Pin the toolbox commit or release and the schema version when you share a model.
