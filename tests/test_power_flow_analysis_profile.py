...
def _fully_specified_network():
    ...
    gen.atNode = bus1
    gen.add_attribute(attribute_id="powerflow_bus_type", value="slack")
...
def test_generation_unit_requires_atnode_bus_type_and_active_power_setpoint():
    model, bus1, _ = _model_with_bus()
    model.add_entity("GenerationUnit", "gen.bare")

    errors = model.validate_for_analysis("power_flow")
    assert any("gen.bare" in e and "atNode" in e for e in errors)
    assert any("gen.bare" in e and "powerflow_bus_type" in e for e in errors)
...
def test_interconnector_and_hvdclink_are_not_checked_for_impedance():
    ...
    power_flow profile only requires their shared fromNode/toNode, not
    an impedance any AC solver would need."""
    model, bus1, bus2 = _model_with_bus()
...
def test_transmission_element_topology_required_for_every_subclass():
    ...
    model.add_entity("Interconnector", "ntc.floating")
