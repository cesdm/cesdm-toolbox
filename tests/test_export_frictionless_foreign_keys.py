...
Fixed by generalising the existing AllAssets union-table pattern: the
...
populated concrete descendant, mirroring exactly how AllAssets already
...
def _model_with_avr_dynamics_and_topology(tmp_path):
    ...
    gen.power_flow.powerflow_bus_type = "PQ"
...
def test_previously_broken_abstract_target_relations_now_have_a_foreign_key(tmp_path):
    ...
    checks = [
        ...
        ("transmission-line", "fromNode", "all-network-node"),
        ("transmission-line", "toNode", "all-network-node"),
    ]
