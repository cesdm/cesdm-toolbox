...
def test_draws_from_reservoir_link_points_at_an_id_that_actually_exists(two_bus_phs_model_yaml):
    ...
    assert agg.dangling_relation_targets(out_obj) == []


def test_draws_from_reservoir_survives_disaggregated_hydro_tech_group(two_bus_phs_model_yaml):
    """Hydro tech-group matches the machines but not the reservoirs
    (HydraulicStorageUnit has no generation-technology id). On a
    disaggregated country the storage loop therefore keeps original
    reservoir ids while the generators still merge per bus -- the
    drawsFromHydraulicStorage target must be an id that was actually
    emitted, and no reservoir may be orphaned."""
    model = agg.load_cesdm_model(ROOT / "schemas/cesdm", two_bus_phs_model_yaml)
    data = agg.model_to_data(model)
    buses = agg.section_items(data, "ElectricalBus")
    ...
    out_obj, _series, stats = agg.aggregate_subset(
        data, buses, "country", True, None, lambda *a: None,
        level_by_country={"de": "disaggregated"},
    ...
    for gen_ent in out_obj["HydroGenerationUnit"].values():
        for rel in gen_ent["relations"]:
            if rel["id"] != "drawsFromHydraulicStorage":
                continue
            for tid in rel["target_entity_ids"]:
                assert tid in actual_reservoir_ids
    linked = {
        tid
        for gen_ent in out_obj["HydroGenerationUnit"].values()
        for rel in gen_ent["relations"]
    ...
    assert actual_reservoir_ids <= linked
    assert stats.get("_storage_integrity_errors", 0) == 0
...
def test_sanity_check_conserves_totals_across_all_categories(tmp_path):
    ...
    bat.dispatch.maximum_discharging_power = 120.0
    ...
    assert sum(disagg["storage_discharge_capacity_mw"].values()) == sum(aggd["storage_discharge_capacity_mw"].values()) == 120.0
...
def test_branch_assets_are_not_merged_or_averaged(tmp_path):
    ...
    assert stats["icos"] == 2
    assert stats["trafos"] == 2
    # in − dropped = out over all four branch classes (nothing dropped here)
    assert stats["lines"] + stats["icos"] + stats["hvdc"] + stats["trafos"] == 8


def test_ndarray_truthiness_does_not_crash_disaggregated_profile_copy(tmp_path):
    """build_subset_disaggregated used `pm.col(pid) or pm.col(...)`, which
    raises on a real (non-empty) ndarray. The success path must copy the
    kept profile into series_dict."""
    import h5py
    import numpy as np
    ...
    model.add_attribute("node.de111.380", "nominal_voltage", 380)
    model.add_relation("node.de111.380", "belongsToGeographicalRegion", "nuts3.de111")
    model.add_entity("TimestampSeries", "timestamp.pypsa")
    model.add_attribute("timestamp.pypsa", "resolution", "PT1H")
    model.add_attribute("timestamp.pypsa", "length", 4)
    model.add_entity("GenerationUnit", "gen.wind.1")
    model.set_technology(
        "gen.wind.1", "Generation.Renewable.Wind.Onshore", technology_class="GeneratorType",
    )
    model.add_relation("gen.wind.1", "atNode", "node.de111.380")
    model.get_entity("gen.wind.1").dispatch.nominal_power_capacity = 100.0
    model.add_entity("Profile", "profile.gen.wind.1")
    model.add_attribute("profile.gen.wind.1", "profile_type", "as_capacity_factor")
    model.add_attribute("profile.gen.wind.1", "data_reference", "profiles.h5:/profiles/profile.gen.wind.1/values")
    model.add_relation("profile.gen.wind.1", "hasTimestampSeries", "timestamp.pypsa")
    model.add_relation("gen.wind.1", "hasAvailabilityProfile", "profile.gen.wind.1")
    _ensure_elec_carrier_domain(model)
    yaml_path = tmp_path / "disagg.yaml"
    model.export_yaml_hierarchical(str(yaml_path))

    h5_path = tmp_path / "profiles.h5"
    with h5py.File(h5_path, "w") as f:
        f.create_dataset(
            "profiles/profile.gen.wind.1/values",
    ...
    buses = agg.section_items(data, "ElectricalBus")
    pm = agg.ProfileMatrix(h5_path)
    out_obj, series, stats = agg.build_subset_disaggregated(
        data, buses, pm, lambda *a: None,
    )
    assert "profile.gen.wind.1" in series
    assert stats["hvdc"] == 0
    assert agg.dangling_relation_targets(out_obj) == []


def test_profile_matrix_reads_hdf5_named_with_profiles_stem(tmp_path):
    import h5py
    import numpy as np

    h5_path = tmp_path / "x_profiles.h5"
    with h5py.File(h5_path, "w") as f:
        f.create_dataset("profiles/p1/values", data=np.array([1.0, 2.0], dtype=np.float32))
    pm = agg.ProfileMatrix(h5_path)
    assert pm._fmt == "cesdm"
    assert pm.T == 2
    col = pm.col("p1")
    assert col is not None
    assert list(col) == pytest.approx([1.0, 2.0])
    pm.close()


def test_all_invalid_keep_selectors_are_rejected():
    valid_nuts3 = ["gr302", "ch021"]
    requested, keep_codes = ["el"], ["EL"]
    valid, invalid = agg.validate_selectors(requested, valid_nuts3)
    assert valid == []
    assert invalid == ["el"]
    with pytest.raises(SystemExit, match="None of --keep"):
        agg.reject_empty_keep_after_validation(requested, valid, keep_codes)


def test_partially_invalid_keep_selectors_still_keep_the_valid_ones():
    valid_nuts3 = ["ch021", "de111"]
    valid, invalid = agg.validate_selectors(["ch", "zz"], valid_nuts3)
    assert valid == ["ch"]
    assert invalid == ["zz"]
    agg.reject_empty_keep_after_validation(["ch", "zz"], valid, ["CH", "ZZ"])


def test_availability_profiles_are_capacity_weighted_when_as_capacity_factor(tmp_path):
    """Members store un-normalized capacity factors. Weighting by
    annual_resource_potential (the old energy_shape shortcut) tilts the
    hourly shape; capacity weights recover Σ_i P_i·cf_i(t)."""
    import h5py
    import numpy as np
    ...
    cf0 = np.array([0.5, 0.5, 0.0, 0.0], dtype=np.float64)
    cf1 = np.array([0.0, 0.0, 0.1, 0.1], dtype=np.float64)
    p0, p1 = 100.0, 100.0
    for i, (cf, cap, arp) in enumerate([
        (cf0, p0, float(p0 * cf0.sum())),
        (cf1, p1, float(p1 * cf1.sum())),
    ]):
        gid = f"gen.onwind.{i}"
        pid = f"profile.{gid}"
        model.add_entity("GenerationUnit", gid)
        model.set_technology(
            gid, "Generation.Renewable.Wind.Onshore", technology_class="GeneratorType",
        )
        model.add_relation(gid, "atNode", "node.de111.380")
        gen = model.get_entity(gid)
        gen.dispatch.nominal_power_capacity = cap
        gen.dispatch.annual_resource_potential = arp
        gen.dispatch.dispatch_type = "nondispatchable"
        model.add_entity("Profile", pid)
        model.add_attribute(pid, "profile_type", "as_capacity_factor")
        model.add_attribute(pid, "data_reference", f"profiles.h5:/profiles/{pid}/values")
        model.add_relation(pid, "hasTimestampSeries", "timestamp.pypsa")
        model.add_relation(gid, "hasAvailabilityProfile", pid)

    _ensure_elec_carrier_domain(model)
    yaml_path = tmp_path / "cf_weight.yaml"
    model.export_yaml_hierarchical(str(yaml_path))

    h5_path = tmp_path / "profiles.h5"
    with h5py.File(h5_path, "w") as f:
        f.create_dataset("profiles/profile.gen.onwind.0/values", data=cf0.astype(np.float32))
        f.create_dataset("profiles/profile.gen.onwind.1/values", data=cf1.astype(np.float32))
    ...
    gen_id = next(iter(out_obj["GenerationUnit"]))
    attrs = {a["id"]: a["value"] for a in out_obj["GenerationUnit"][gen_id]["attributes"]}
    ann_sum = float(attrs["annual_resource_potential"])
    pid = next(
        r["target_entity_ids"][0]
        for r in out_obj["GenerationUnit"][gen_id]["relations"]
        if r["id"] == "hasAvailabilityProfile"
    )
    agg_arr = np.asarray(series[pid], dtype=np.float64)
    expected = cf0 * p0 + cf1 * p1
    np.testing.assert_allclose(agg_arr * ann_sum, expected, rtol=1e-5, atol=1e-5)

    rels = {r["id"]: r["target_entity_ids"][0] for r in out_obj["Profile"][pid]["relations"]}
    assert rels["hasTimestampSeries"] in out_obj["TimestampSeries"]
    assert rels["hasTimestampSeries"] == "timestamp.pypsa"

    de = stats["_sanity_aggregated_by_country"]["de"]
    dis = stats["_sanity_disaggregated_by_country"]["de"]
    assert de["nondispatchable_peak_available_mw"]
    assert dis["nondispatchable_peak_available_mw"]
    tech = next(iter(de["nondispatchable_peak_available_mw"]))
    assert de["nondispatchable_peak_available_mw"][tech] == pytest.approx(
        dis["nondispatchable_peak_available_mw"][tech], rel=1e-5,
    )


def test_merged_ramp_rate_is_capacity_weighted_mean(tmp_path):
    model = build_model_from_yaml(str(ROOT / "schemas/cesdm"))
    model.import_library(str(ROOT / "library" / "default_library"))
    model.add_entity("GeographicalRegion", "nuts3.de111")
    model.add_entity("ElectricalBus", "node.de111.380")
    model.add_attribute("node.de111.380", "nominal_voltage", 380)
    model.add_relation("node.de111.380", "belongsToGeographicalRegion", "nuts3.de111")
    for i, (cap, ramp) in enumerate([(100.0, 0.05), (50.0, 0.02)]):
        gid = f"gen.nuc.{i}"
        model.add_entity("GenerationUnit", gid)
        model.set_technology(gid, "Generation.Nuclear.LWR", technology_class="GeneratorType")
        model.add_relation(gid, "atNode", "node.de111.380")
        gen = model.get_entity(gid)
        gen.dispatch.nominal_power_capacity = cap
        gen.dispatch.maximum_ramp_rate_up = ramp
        gen.dispatch.maximum_ramp_rate_down = ramp
        gen.dispatch.dispatch_type = "dispatchable"
    _ensure_elec_carrier_domain(model)
    yaml_path = tmp_path / "ramp.yaml"
    model.export_yaml_hierarchical(str(yaml_path))
    ...
    gen_id = next(iter(out_obj["GenerationUnit"]))
    attrs = {a["id"]: a["value"] for a in out_obj["GenerationUnit"][gen_id]["attributes"]}
    expected = (100.0 * 0.05 + 50.0 * 0.02) / 150.0
    assert attrs["maximum_ramp_rate_up"] == pytest.approx(expected)
    assert attrs["maximum_ramp_rate_down"] == pytest.approx(expected)
    dis = stats["_sanity_disaggregated_by_country"]["de"]
    aggd = stats["_sanity_aggregated_by_country"]["de"]
    assert sum(dis["dispatchable_ramp_up_mw_pct"].values()) == pytest.approx(100.0 * 0.05 + 50.0 * 0.02)
    assert sum(aggd["dispatchable_ramp_up_mw_pct"].values()) == pytest.approx(
        sum(dis["dispatchable_ramp_up_mw_pct"].values())
    )


def test_hydro_availability_profile_survives_data_to_model(tmp_path):
    """A reservoir-less HydroGenerationUnit carries hasAvailabilityProfile.
    After country-level merge the relation must still be hasAvailabilityProfile
    and must survive the data_to_model round-trip (hasNaturalInflowProfile
    is undeclared on the machine and would be silently dropped)."""
    import h5py
    import numpy as np
    ...
    model.import_library(str(ROOT / "library" / "default_library"))
    model.ensure_resource("resource.water", name="Water", resource_type="water")
    for i, rid in enumerate(["nuts3.de111", "nuts3.de112"]):
        model.add_entity("GeographicalRegion", rid)
        bid = f"node.de11{i+1}.380"
        model.add_entity("ElectricalBus", bid)
        model.add_attribute(bid, "nominal_voltage", 380)
        model.add_relation(bid, "belongsToGeographicalRegion", rid)
    model.add_entity("TimestampSeries", "ts.hourly")
    model.add_attribute("ts.hourly", "resolution", "PT1H")
    model.add_attribute("ts.hourly", "length", 2)
    for i, bid in enumerate(["node.de111.380", "node.de112.380"]):
        gid = f"gen.hydro.{i}"
        pid = f"profile.{gid}"
        model.add_entity("HydroGenerationUnit", gid)
        model.set_technology(
            gid, "Generation.Renewable.Hydro.RunOfRiver", technology_class="GeneratorType",
        )
        model.add_relation(gid, "atNode", bid)
        model.add_relation(gid, "hasInputResource", "resource.water")
        gen = model.get_entity(gid)
        gen.dispatch.nominal_power_capacity = 40.0
        gen.dispatch.dispatch_type = "nondispatchable"
        model.add_entity("Profile", pid)
        model.add_attribute(pid, "profile_type", "as_capacity_factor")
        model.add_attribute(pid, "data_reference", f"profiles.h5:/profiles/{pid}/values")
        ...
        model.add_relation(gid, "hasAvailabilityProfile", pid)
    _ensure_elec_carrier_domain(model)
    yaml_path = tmp_path / "hydro_av.yaml"
    model.export_yaml_hierarchical(str(yaml_path))

    h5_path = tmp_path / "profiles.h5"
    with h5py.File(h5_path, "w") as f:
        f.create_dataset("profiles/profile.gen.hydro.0/values", data=np.array([0.2, 0.3]))
        f.create_dataset("profiles/profile.gen.hydro.1/values", data=np.array([0.4, 0.5]))
    ...
    hid = next(iter(out_obj["HydroGenerationUnit"]))
    rel_ids = {r["id"] for r in out_obj["HydroGenerationUnit"][hid]["relations"]}
    assert "hasAvailabilityProfile" in rel_ids
    assert "hasNaturalInflowProfile" not in rel_ids

    out_model = agg.data_to_model(ROOT / "schemas/cesdm", out_obj)
    hydro = next(iter(out_model.entities["HydroGenerationUnit"].values()))
    stored = hydro.data.get("hasAvailabilityProfile")
    value = stored[0] if isinstance(stored, list) else stored
    assert value, f"hasAvailabilityProfile lost in data_to_model; data={hydro.data}"


def test_region_parents_are_carried_for_disaggregated_buses(tmp_path):
    model = build_model_from_yaml(str(ROOT / "schemas/cesdm"))
    model.import_library(str(ROOT / "library" / "default_library"))
    model.add_entity("GeographicalRegion", "nuts2.de11")
    model.add_attribute("nuts2.de11", "name", "DE NUTS2")
    model.add_entity("GeographicalRegion", "nuts3.de111")
    model.add_attribute("nuts3.de111", "name", "DE NUTS3")
    model.add_relation("nuts3.de111", "isSubRegionOf", "nuts2.de11")
    model.add_entity("ElectricalBus", "node.de111.380")
    model.add_attribute("node.de111.380", "nominal_voltage", 380)
    model.add_relation("node.de111.380", "belongsToGeographicalRegion", "nuts3.de111")
    model.add_entity("GenerationUnit", "gen.de")
    model.set_technology(
        "gen.de", "Generation.Thermal.Gas.CCGT.New", technology_class="GeneratorType",
    )
    model.add_relation("gen.de", "atNode", "node.de111.380")
    model.get_entity("gen.de").dispatch.nominal_power_capacity = 10.0
    _ensure_elec_carrier_domain(model)
    ...
    geo = out_obj["GeographicalRegion"]
    assert "nuts3.de111" in geo
    assert "nuts2.de11" in geo
    assert agg.dangling_relation_targets(out_obj) == []


def test_passthrough_assets_keep_resolvable_has_technology(two_country_model_yaml):
    model = agg.load_cesdm_model(ROOT / "schemas/cesdm", two_country_model_yaml)
    data = agg.model_to_data(model)
    buses = agg.section_items(data, "ElectricalBus")
    out_obj, _series, _stats = agg.aggregate_subset(
    ...
    assert out_obj.get("GeneratorType")
    dangling = agg.dangling_relation_targets(out_obj)
    assert dangling == [], dangling


def test_mixed_voltage_merge_warns_and_keeps_max_kv(tmp_path):
    model = build_model_from_yaml(str(ROOT / "schemas/cesdm"))
    model.add_entity("GeographicalRegion", "nuts3.de111")
    model.add_entity("ElectricalBus", "node.de111.220")
    model.add_attribute("node.de111.220", "nominal_voltage", 220)
    model.add_relation("node.de111.220", "belongsToGeographicalRegion", "nuts3.de111")
    model.add_entity("ElectricalBus", "node.de111.380")
    model.add_attribute("node.de111.380", "nominal_voltage", 380)
    model.add_relation("node.de111.380", "belongsToGeographicalRegion", "nuts3.de111")
    _ensure_elec_carrier_domain(model)
    yaml_path = tmp_path / "kv.yaml"
    model.export_yaml_hierarchical(str(yaml_path))
    ...
    logged = []
    out_obj, _series, _stats = agg.aggregate_subset(
        data, buses, "country", False, None, logged.append,
    )
    bus = next(iter(out_obj["ElectricalBus"].values()))
    attrs = {a["id"]: a["value"] for a in bus["attributes"]}
    assert attrs["nominal_voltage"] == pytest.approx(380.0)
    assert any("merges voltage levels" in m for m in logged)


def test_missing_member_profile_emits_a_warning(tmp_path):
    import h5py
    import numpy as np
    ...
    for i in range(2):
        ...
        model.add_relation(gid, "atNode", "node.de111.380")
        gen = model.get_entity(gid)
        gen.dispatch.nominal_power_capacity = 100.0
        gen.dispatch.dispatch_type = "nondispatchable"
        model.add_entity("Profile", pid)
        model.add_attribute(pid, "profile_type", "as_capacity_factor")
        model.add_attribute(pid, "data_reference", f"profiles.h5:/profiles/{pid}/values")
        ...
        model.add_relation(gid, "hasAvailabilityProfile", pid)
    _ensure_elec_carrier_domain(model)
    yaml_path = tmp_path / "missing_prof.yaml"
    model.export_yaml_hierarchical(str(yaml_path))

    h5_path = tmp_path / "profiles.h5"
    with h5py.File(h5_path, "w") as f:
        f.create_dataset("profiles/profile.gen.onwind.0/values", data=np.array([0.2, 0.3]))
        # profile.gen.onwind.1 deliberately absent
    ...
    logged = []
    pm = agg.ProfileMatrix(h5_path)
    agg.aggregate_subset(data, buses, "country", True, pm, logged.append)
    assert any("missing profile in source h5" in m and "profile.gen.onwind.1" in m for m in logged)


def test_merged_storage_carries_maximum_discharging_power(tmp_path):
    model = build_model_from_yaml(str(ROOT / "schemas/cesdm"))
    model.import_library(str(ROOT / "library" / "default_library"))
    model.add_entity("GeographicalRegion", "nuts3.de111")
    model.add_entity("ElectricalBus", "node.de111.380")
    ...
    sid = next(iter(out_obj["StorageUnit"]))
    attrs = {a["id"]: a["value"] for a in out_obj["StorageUnit"][sid]["attributes"]}
    assert attrs["maximum_discharging_power"] == pytest.approx(120.0)
