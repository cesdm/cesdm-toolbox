...
def test_hydro_reservoir_composite_gives_generator_not_reservoir_the_topology_view():
    ...
    model.add_entity("ReservoirStorageUnit", reservoir_id)
    ...
    assert gen_ent.data.get("drawsFromReservoir") == reservoir_id
    res_ent = model.entities["ReservoirStorageUnit"][reservoir_id]
    assert res_ent.data.get("suppliesResourceTo") == gen_id
