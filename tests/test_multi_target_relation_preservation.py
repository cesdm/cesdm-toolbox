...
Found via tools/aggregate_cesdm_yaml_subset.py's per-country technology
...
same relation id (HydroGenerationUnit.drawsFromReservoir, after
...
def _build_model_with_multi_target_relation():
    ...
    model.add_entity("ReservoirStorageUnit", "res.a")
    model.add_attribute("res.a", "name", "res.a")
    model.add_entity("ReservoirStorageUnit", "res.b")
    model.add_attribute("res.b", "name", "res.b")
    model.add_entity("HydroGenerationUnit", "gen.1")
    model.add_attribute("gen.1", "name", "gen.1")
    model.add_relation("gen.1", "drawsFromReservoir", "res.a")
    model.add_relation("gen.1", "drawsFromReservoir", "res.b")
    return model
...
def test_add_relation_itself_has_no_accumulation_semantics():
    ...
    *sets*, never *appends* -- calling it twice for the same
    ...
    stored = model.entities["HydroGenerationUnit"]["gen.1"].data.get("drawsFromReservoir")
...
def test_import_yaml_hierarchical_preserves_every_target(tmp_path):
    ...
    # tools/aggregate_cesdm_yaml_subset.py's data_to_model() now does.
    model.entities["HydroGenerationUnit"]["gen.1"].data["drawsFromReservoir"] = ["res.a", "res.b"]
    ...
    stored = reloaded.entities["HydroGenerationUnit"]["gen.1"].data.get("drawsFromReservoir")
