...
(representsAsset/controlsGenerationUnit), not representation views
...
def _build_bare_asset_with_linked_result():
    ...
    model.add_relation("gen.bare.result", "representsAsset", "gen.bare")
...
def test_bare_asset_with_linked_result_round_trips(tmp_path):
    ...
    result = next((r for r in results.values() if r.data.get("representsAsset") == "gen.bare"), None)
