...
def test_fully_specified_model_passes_with_zero_errors():
    ...
    gas = _add_generator(model, "gas.1", technology="Generation.Thermal.Gas.CCGT.Present2", bus=bus1)
...
def test_load_analysis_profile_by_explicit_path():
    ...
    # GenerationUnit, DemandUnit, WindGenerationUnit,
    # SolarGenerationUnit, HydroGenerationUnit, StorageUnit,
    # ReservoirStorageUnit, TransmissionElement (shared fromNode/toNode),
    # CHPUnit, plus one block per concrete transmission subclass
    # (TransmissionLine, Transformer, Interconnector, HVDCLink) for
    # their own capacity field.
...
def test_ambiguous_attribute_without_view_family_gives_a_clear_error_not_a_crash():
    ...
    model, bus1, bus2 = _model_with_bus()
    _add_generator(model, "gas.1", technology="Generation.Thermal.Gas.CCGT.Present2", bus=bus1)
...
def test_controller_attribute_resolves_automatically_via_view_family():
    ...
    representsAsset -- so _discover_view_map() (keyed on
    representsAsset) could never see them, and a profile check for a
...
def test_missing_controller_is_reported_as_a_clear_error_not_a_crash():
    ...
    raise."""
    model, bus1, _ = _model_with_bus()
    _add_generator(model, "gas.no_avr", technology="Generation.Thermal.Gas.CCGT.Present2", bus=bus1)
...
def test_result_entity_attribute_still_does_not_resolve():
    ...
    model.add_relation("result.run1.gas1", "representsAsset", "gas.1")
...
def test_missing_dynamic_model_is_reported_as_a_clear_error_not_a_crash():
    model, bus1, _ = _model_with_bus()
    _add_generator(model, "gas.no_dyn", technology="Generation.Thermal.Gas.CCGT.Present2", bus=bus1)
