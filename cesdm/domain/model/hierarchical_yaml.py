...
class HierarchicalYamlMixin:
    ...
    def export_yaml_hierarchical(self, path: str | pathlib.Path) -> None:
        ...
        etc.), the only classes left using the representsAsset pattern --
        are nested under a ``representations`` key, grouped by class
        name. The ``representsAsset`` back-relation is omitted from each
        ...
        def _ingest_rels(cls, eid, rels_block, skip=None):
            ...
            for rname, rid in items:
                ...
                if rname in known_refs.get(cls, set()):
                    ...
                    # drawsFromReservoir after spatial/technology
