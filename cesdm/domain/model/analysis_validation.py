...
- **`Result` entities** (`GenerationUnit.DispatchResult`, ...), linked
  via `representsAsset` -- discovered through `_discover_view_map()`.
...
class CesdmAnalysisValidationMixin:
    ...
    #: model). These are linked this way, not via `representsAsset`, so
    #: `_discover_view_map()` (which is keyed on `representsAsset`) can
    ...
    def _find_existing_view_for_family(self, entity_id: str, family: str) -> Optional[str]:
        ...
        Tries `representsAsset`-linked entities (Result) first,
    ...
    def _find_view_family_for_attribute(self, entity_class: str, attribute: str) -> Optional[str]:
        ...
        Searches both `representsAsset`-linked classes (Result
