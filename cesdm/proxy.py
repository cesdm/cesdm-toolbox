...
def _known_view_families(model: Any) -> set:
    ...
    return {"dispatch", "topology", "power_flow", "spatial", "planning", "technical", "dynamics"}
...
class FlatGroupViewProxy:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        model = self._model
        if name in rels:
            targets = model.get_relation_targets(self._asset_id, name)
            return _relation_value(model, targets)
        if name in attrs:
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        model = self._model
        if name in rels:
            model.add_relation_if_allowed(self._asset_id, name, value, strict=True)
            return
        if name in attrs:
            unit = None
            if isinstance(value, tuple) and len(value) == 2:
...
class EntityProxy(_EarEntityProxy):
    ...
    - Reading a relation returns the target wrapped in its own typed
      proxy (e.g. `bus.locatedIn` returns a `GeographicalRegionProxy`,
      not a plain string) -- ear.entity_proxy.EntityProxy's generic
      `__getattr__` returns plain target ids instead, since a
      non-CESDM EAR domain may have no per-class proxy registry to
      wrap with at all.
    ...
    def _known_names(self) -> set:
        ...
        return _known_view_families(model) | super()._known_names() if cname else _known_view_families(model)
    ...
    _KNOWN_GROUPS = {"dispatch", "topology", "power_flow", "spatial", "planning", "technical", "dynamics"}
