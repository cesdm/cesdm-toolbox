...
def transmission_lines_summary(model: CesdmModel) -> Dict[str, Any]:
    ...
    Data sources
    ------------
    thermal_capacity_rating → TransmissionLine  [MVA]  (per-circuit rating)
    line_length             → TransmissionLine  [km]
    parallel_circuit_count  → TransmissionLine

    Corridor totals use rating × parallel_circuit_count and
    length × parallel_circuit_count (CESDM stores single-circuit thermal
    rating and circuit count separately).
