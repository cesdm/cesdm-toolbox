...
# Fallback line-type parameters when network.line_types is unavailable
# (r, x in Ω/km; b in μS/km). Values match PyPSA's built-in Al/St 240/40
# types: b = 2*π*f*c_per_length with f=50 Hz and c in nF/km.
_LINE_TYPE_PARAMS: Dict[str, Dict[str, float]] = {
    "Al/St 240/40 2-bundle 220.0": {"r": 0.060, "x": 0.301, "b": 3.927},
    "Al/St 240/40 3-bundle 300.0": {"r": 0.040, "x": 0.265, "b": 4.147},
    "Al/St 240/40 4-bundle 380.0": {"r": 0.030, "x": 0.246, "b": 4.335},
}

# Voltage-class defaults used only when neither line.type nor line_types
# provide per-km parameters (x from the Al/St types above; r/b historically 0).
_LINE_X_DEFAULT_BY_KV: Dict[int, float] = {220: 0.301, 300: 0.265, 380: 0.246}
_LINE_S_DEFAULT_BY_KV: Dict[int, float] = {
    220: 491.556019188047,
    300: 1005.45549379373,
    380: 1698.10261174053,
}


def _c_nf_per_km_to_b_micro_s_per_km(c_nf_per_km: float, f_hz: float = 50.0) -> float:
    """Convert line-type shunt capacitance [nF/km] to susceptance [μS/km]."""
    # B[S/km] = 2*π*f*C[F/km]; C[F/km] = c_nf_per_km * 1e-9
    # B[μS/km] = B[S/km] * 1e6 = 2*π*f*c_nf_per_km * 1e-3
    return float(2.0 * math.pi * f_hz * c_nf_per_km * 1e-3)


def _line_rxb_per_km(
    network: pypsa.Network,
    line,
    v_nom_line: float,
) -> Tuple[float, float, float]:
    """Return (r, x, b) per km for a PyPSA line.

    In typical PyPSA-Eur ``elec.nc`` files, ``lines.r`` / ``lines.x`` are zero;
    the physical per-km values live on the standard type library
    (``network.line_types``), keyed by ``lines.type``:
    ``r_per_length`` / ``x_per_length`` [Ω/km], ``c_per_length`` [nF/km].

    Preference order:
      1. ``network.line_types`` row for ``line.type``
      2. built-in ``_LINE_TYPE_PARAMS`` for known Al/St type names
      3. voltage-class x default; r=0, b=0
    """
    type_name = getattr(line, "type", None)
    type_key = str(type_name).strip() if type_name is not None else ""
    if type_key.lower() in ("", "nan"):
        type_key = ""

    if type_key and hasattr(network, "line_types") and type_key in network.line_types.index:
        row = network.line_types.loc[type_key]
        r = float(row["r_per_length"]) if "r_per_length" in row.index else 0.0
        x = float(row["x_per_length"]) if "x_per_length" in row.index else 0.0
        f_hz = float(row["f_nom"]) if "f_nom" in row.index and row["f_nom"] else 50.0
        if "c_per_length" in row.index and row["c_per_length"] is not None:
            try:
                c = float(row["c_per_length"])
                b = (
                    _c_nf_per_km_to_b_micro_s_per_km(c, f_hz)
                    if not math.isnan(c) else 0.0
                )
    ...
    if type_key and type_key in _LINE_TYPE_PARAMS:
        p = _LINE_TYPE_PARAMS[type_key]
        return float(p["r"]), float(p["x"]), float(p["b"])

    v_std = min(_LINE_X_DEFAULT_BY_KV.keys(), key=lambda v: abs(v - v_nom_line))
    return 0.0, float(_LINE_X_DEFAULT_BY_KV[v_std]), 0.0
...
def build_cesdm_from_pypsa(
    ...
    for line_id, line in network.lines.iterrows():
        ...
        #
        # In PyPSA-Eur elec.nc, lines.r / lines.x are typically zero; the
        # physical per-km values live on network.line_types (keyed by
        # lines.type): r_per_length / x_per_length [Ω/km], c_per_length
        # [nF/km] → CESDM shunt_susceptance_per_km [μS/km].
        # thermal_capacity_rating stays the per-circuit voltage-class
        # default (parallel_circuit_count is stored separately).
        ...
        r_per_km, x_per_km, b_per_km = _line_rxb_per_km(network, line, v_nom_line)

        v_std = min(_LINE_S_DEFAULT_BY_KV.keys(), key=lambda v: abs(v - v_nom_line))
        ...
        s_default = _LINE_S_DEFAULT_BY_KV.get(v_std, 1698.10261174053)
