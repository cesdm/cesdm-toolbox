...
def _is_reservoir_hydro_storage(carrier: str | None, technology: str | None = None) -> bool:
    ...
    battery-like StorageUnit: it is represented as a ReservoirStorageUnit plus a
...
def build_cesdm_from_pypsa(
    ...
    if bus_country_col is None:
        ...
        model.add_entity(entity_class="GeographicalRegion", entity_id=default_region_id)
            ...
            if bus_country_col is not None:
                ...
                if ccode and ccode.lower() != "nan":
                    if ccode not in region_by_code:
                        ...
                        model.add_entity(entity_class="GeographicalRegion", entity_id=gr_id)
    ...
    for gen_id, gen in network.generators.iterrows():
        ...
        # later -- by tools/aggregate_cesdm_yaml_subset.py's own "no
        ...
        if has_ts_profile:
            ...
            # annual_resource_potential + hasAvailabilityProfile, held directly on the asset
    ...
    for su_id, su in network.storage_units.iterrows():
        ...
        # anything that assumes a ReservoirStorageUnit has no direct
        ...
        # StorageUnit and ReservoirStorageUnit intentionally have different
    ...
    if not network.links.empty:
        for link_id, link in network.links.iterrows():
            ...
            if same_carrier:
                ...
                model.add_entity(entity_class="ConversionUnit", entity_id=eid)
                safe_set_attr(model, eid, "name", eid)
