...
# Technology-library and model-metadata classes. Passthrough assets keep
# hasTechnology pointers into these; they must be copied into the output
# or those relations dangle.
LIBRARY_SECTIONS = (
    "GeneratorType",
    "StorageType",
    "ConverterType",
...
def aggregated_storage_id_for_asset(
    ...
    for asset_class, members in sto_by_class.items():
        ...
        # Disaggregated passthrough: the storage loop keeps the original id
        # when no --tech-group pattern matches. Mirror that here so hydro
        # machines do not point at a minted id that was never created.
        if bus_id == agg_bus and not any(
            technology_matches_group(str(tech_raw), p) for p in tech_groups
        ):
            return asset_id
        tech = technology_tag_for_groups(str(tech_raw), tech_groups)
        tag = id_tag(tech)
...
def generation_profile_relation_for_bucket(bucket: str) -> str:
    # Natural inflow belongs on HydraulicStorageUnit (hasNaturalInflowProfile),
    # never on HydroGenerationUnit. A reservoir-less hydro machine that carries
    # an availability series must keep hasAvailabilityProfile through merge.
...
def reject_empty_keep_after_validation(
    requested: List[str], valid: List[str], keep_codes: List[str]
) -> None:
    """Fail if the user passed --keep codes but none of them match the model.

    An empty selector list means "keep everything". Collapsing an all-invalid
    --keep set into that meaning would silently emit a continent-wide model.
    """
    if requested and not valid:
        raise SystemExit(
            f"None of --keep {keep_codes} matches any region code in the model "
            f"(this model uses GB/GR, not the NUTS spellings UK/EL)"
        )
    ...
    for cls_name, entities in data.items():
        ...
        for eid, ent in entities.items():
            ...
            try:
                ...
                if log is not None:
                    log(f"[WARN] data_to_model: skip entity {cls_name}:{eid}: {exc}")
            ...
            for a in ent.get("attributes", []) or []:
                ...
                try:
                    ...
                    if log is not None:
                        log(f"[WARN] data_to_model: skip attribute {cls_name}:{eid}.{aid}: {exc}")
            for r in ent.get("relations", []) or []:
                ...
                try:
                    ...
                    if log is not None:
                        log(f"[WARN] data_to_model: skip relation {cls_name}:{eid}.{rid}: {exc}")
...
class ProfileMatrix:
    ...
    def __init__(self, path: Path):
        path = Path(path)
        # Content first: a CESDM HDF5 named ``*_profiles.h5`` (this tool's own
        # default stem) must not be dispatched to the Parquet reader.
        if path.exists() and h5py.is_hdf5(path):
            ...
            if self._fmt == "flat":
                ...
                self._init_cesdm()
        elif path.suffix.lower() == ".parquet" or "_profiles" in path.stem:
            self._fmt = "parquet"
...
def library_sections(data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """Copy technology-library / model-metadata classes from the input."""
    return {cls: section_items(data, cls) for cls in LIBRARY_SECTIONS}


def output_timestamp_series_id(data: Dict[str, Any]) -> str:
    ts_sec = section_items(data, "TimestampSeries")
    return next(iter(ts_sec), TIMESTAMP_SERIES_ID)


def close_region_parents(
    out_geo: Dict[str, Dict[str, Any]],
    geo_sec: Dict[str, Dict[str, Any]],
) -> None:
    """Pull in every isSubRegionOf ancestor so copied regions do not dangle."""
    frontier = set(out_geo)
    while frontier:
        nxt: set = set()
        for gid in frontier:
            for t in get_rels(out_geo.get(gid, {})).get("isSubRegionOf", []):
                if t not in out_geo and t in geo_sec:
                    out_geo[t] = copy.deepcopy(geo_sec[t])
                    nxt.add(t)
        frontier = nxt


def dangling_relation_targets(out_obj: Dict[str, Any]) -> List[Tuple[str, str, str, str]]:
    """Return (class, entity_id, relation, target) for targets missing from out_obj."""
    all_ids = {
        eid
        for sec in out_obj.values()
        if isinstance(sec, dict)
        for eid in sec
    }
    dangling: List[Tuple[str, str, str, str]] = []
    for cls_name, sec in out_obj.items():
        if not isinstance(sec, dict):
            continue
        for eid, ent in sec.items():
            if not isinstance(ent, dict):
                continue
            for rid, targets in get_rels(ent).items():
                for t in targets:
                    if t not in all_ids:
                        dangling.append((cls_name, eid, rid, t))
    return dangling


def log_dangling_relation_targets(out_obj: Dict[str, Any], log) -> int:
    items = dangling_relation_targets(out_obj)
    for cls_name, eid, rid, t in items:
        log(f"[WARN] dangling relation {cls_name}:{eid} {rid} -> {t}")
    return len(items)
...
def build_subset_disaggregated(
    ...
    for gid in sorted(geo_ids):
        src = geo_sec.get(gid)
        out_geo[gid] = copy.deepcopy(src) if src is not None else {
            "attributes": [make_attr("name", gid)], "relations": [],
        }
    close_region_parents(out_geo, geo_sec)
    ...
    if pm is not None:
        for pid, pent in out_profiles.items():
            ...
            if isinstance(dref, str):
                ...
                col = pm.col(pid)
                if col is None:
                    col = pm.col(dref.split(":")[-1].lstrip("/"))
                if col is not None:
    ...
    stats["_dangling_relations"] = log_dangling_relation_targets(out_obj, log)
...
def summarize_by_country(
    ...
    a2n: Dict[str, str],
    reservoir_bus: Dict[str, str],
    pm: Optional[ProfileMatrix] = None,
    ...
    def _country_totals() -> Dict[str, Any]:
        return {
            ...
            "dispatchable_ramp_up_mw_pct": defaultdict(float),
            "nondispatchable_peak_available_mw": defaultdict(float),
    ...
    profiles_sec = section_items(data, "Profile")
    peak_hourly: Dict[Tuple[str, str], np.ndarray] = {}

    def _hourly_available_mw(ent: Dict[str, Any]) -> Optional[np.ndarray]:
        pid = dispatch_profile_rel(ent)
        if not pid:
            return None
        arr = None
        if series_dict and pid in series_dict:
            arr = np.asarray(series_dict[pid], dtype=np.float64)
        elif pm is not None:
            col = pm.col(pid)
            if col is not None:
        ...
        if arr is None:
            return None
        ptype = str(attr_value(profiles_sec.get(pid, {}), "profile_type") or "").strip().lower()
        if ptype == "as_normalized_annual_energy":
            scale = attr_float(ent, "annual_resource_potential") or 0.0
        else:
            scale = attr_float(ent, "nominal_power_capacity") or 0.0
        if scale <= 0:
            return None
        return arr * scale
    ...
    for asset_class in GENERATION_ASSET_CLASSES:
        for asset_id, ent in section_items(data, asset_class).items():
            ...
            if dispatch_type == "nondispatchable":
                ...
                totals[country]["nondispatchable_annual_resource_potential_mwh"][tech] += attr_float(ent, "annual_resource_potential") or 0.0
                hourly = _hourly_available_mw(ent)
                if hourly is not None:
                    key = (country, tech)
                    if key in peak_hourly:
                        n = min(len(peak_hourly[key]), len(hourly))
                        peak_hourly[key] = peak_hourly[key][:n] + hourly[:n]
                    else:
                        peak_hourly[key] = hourly.copy()
            else:
                totals[country]["dispatchable_capacity_mw"][tech] += cap
                ramp = attr_float(ent, "maximum_ramp_rate_up")
                if ramp is not None:
                    totals[country]["dispatchable_ramp_up_mw_pct"][tech] += cap * ramp

    for (country, tech), hourly in peak_hourly.items():
        totals[country]["nondispatchable_peak_available_mw"][tech] = float(np.max(hourly)) if len(hourly) else 0.0
    ...
    for asset_id, ent in section_items(data, STORAGE_NONRESERVOIR_CLASS).items():
        ...
        nom = attr_float(ent, "nominal_power_capacity") or 0.0
        dis = attr_float(ent, "maximum_discharging_power") or 0.0
        totals[country]["storage_discharge_capacity_mw"][tech] += max(nom, dis)
...
def write_sanity_check_report(
    ...
    dict_keys = [
        ...
        "dispatchable_ramp_up_mw_pct",
        "nondispatchable_peak_available_mw",
...
def aggregate_subset(
    ...
    profiles_sec = section_items(data, "Profile")
    out_ts_id = output_timestamp_series_id(data)
    ...
    for agg_id, members in agg_members.items():
        ...
        distinct_kv = sorted({x for x in kvs if x is not None})
        kv_merged = kv_out if kv_out is not None else (max(distinct_kv) if distinct_kv else None)
        if kv_out is None and len(distinct_kv) > 1:
            log(
                f"[WARN] aggregated bus {agg_id} merges voltage levels {distinct_kv}; "
                f"nominal_voltage set to {kv_merged} — consider --split-voltage"
            )
    ...
    def load_profile(ent: Dict[str, Any], asset_id: Optional[str] = None) -> Optional[np.ndarray]:
        pid = dispatch_profile_rel(ent)
        ...
        dropped = (
            attr_float(ent, "nominal_power_capacity")
            or attr_float(ent, "annual_energy_demand")
            or attr_float(ent, "annual_natural_inflow_energy")
        )
        log(
            f"[WARN] missing profile in source h5: {pid} "
            f"(asset={asset_id or attr_value(ent, 'name')}; "
            f"dropped from merged shape, scalar still includes {dropped})"
        )
        return None
    ...
    for agg_bus, members in load_groups.items():
        ...
        profs = [
            ...
            for aid, e in members
            if (arr := load_profile(e, aid)) is not None
        ]
        ...
        if profs:
            ...
            if W.sum() > 0:
                ...
                out_profiles[new_pid] = make_profile_entity(
                    ...
                    h5_path_relative, ts_id=out_ts_id,
                )
    ...
    emitted_storage_id: Dict[str, str] = {}  # original asset id -> output id
        ...
        for asset_id, ent in members_by_id.items():
            ...
            if _is_disagg_bus(bus_id):
                ...
                if not any(
                    ...
                    emitted_storage_id[asset_id] = asset_id
                    _carry_profile_entity(ent)
                    continue
            tech = (
                attr_value(ent, "storage_technology_type")
    ...
    for (asset_class, agg_bus, tech), members in sto_groups.items():
        ...
        for orig_id, _e in members:
            emitted_storage_id[orig_id] = sid
        ...
        pdis    = [attr_float(e, "maximum_discharging_power")      for _, e in members]
        ...
        pdis_sum   = sum(x or 0.0 for x in pdis)
        ...
        if asset_class == STORAGE_NONRESERVOIR_CLASS:
            ...
            if pdis_sum > 0:
                sto_attrs.append(make_attr("maximum_discharging_power", float(pdis_sum), "MW"))
        ...
        if inflow_sum > 0:
            profs = []
            weights = []
            for orig_id, e in members:
                arr = load_profile(e, orig_id)
                if arr is None:
                    continue
    ...
    for (bucket, agg_bus, tech), members in gen_groups.items():
        ...
        for aid in ("maximum_ramp_rate_up", "maximum_ramp_rate_down"):
            if aid not in allowed:
                continue
            v = wavg([attr_float(e, aid) for _, e in members], caps)
            if v is not None:
                gen_attrs.append(make_attr(aid, float(v), "%/h"))
        for aid in ("minimum_generation", "maximum_generation"):
            if aid not in allowed:
                continue
            vals = [attr_float(e, aid) for _, e in members]
            if any(v is not None for v in vals):
                gen_attrs.append(make_attr(aid, float(sum(v or 0.0 for v in vals)), "MW"))
        ...
        for orig_id, e in members:
            ...
            if arr is None:
                continue
            pid = dispatch_profile_rel(e)
            ptype = str(attr_value(profiles_sec.get(pid, {}), "profile_type") or "").strip().lower()
            if ptype == "as_normalized_annual_energy":
                w = float(attr_float(e, "annual_resource_potential") or 0.0)
            else:
                w = float(attr_float(e, "nominal_power_capacity") or 0.0)
            if w > 0:
                profs.append(arr)
                weights.append(w)
        ...
        if profs and sum(weights) > 0:
            ...
            out_profiles[new_pid] = make_profile_entity(
                ...
                "pu", h5_path_relative, ts_id=out_ts_id,
            )
        ...
        if bucket == "hydro":
            ...
            for original_asset_id, _e in members:
                ...
                if not res_id:
                    continue
                agg_res_id = emitted_storage_id.get(res_id)
                if agg_res_id is None:
    ...
    if reservoir_class_ids:
        ...
        for orphan_id in sorted(orphaned):
            ...
            # was actually emitted for it? A generator that WAS paired
            # pre-aggregation, but whose emitted id doesn't match orphan_id,
            # is a real aggregation bug -- a mismatch, not a source-data gap.
            ...
            for orig_res_id in original_ids:
                ...
                for gen_id in paired_gens:
                    recomputed = emitted_storage_id.get(orig_res_id)
    ...
    for rc in sorted(region_codes):
        ...
        if gid not in out_geo:
            if gid in geo_sec:
                out_geo[gid] = copy.deepcopy(geo_sec[gid])
            else:
    ...
    disagg_totals = summarize_by_country(
        data, bus_country, a2n, reservoir_bus, pm=pm,
    )
    ...
    agg_totals = summarize_by_country(
        out_obj, agg_bus_country, agg_a2n, agg_reservoir_bus,
        pm=pm, series_dict=series_dict,
    )
    stats["_sanity_disaggregated_by_country"] = disagg_totals
    stats["_sanity_aggregated_by_country"] = agg_totals
    stats["_storage_integrity_errors"] = storage_integrity_errors
    stats["_dangling_relations"] = log_dangling_relation_targets(out_obj, log)
...
def write_summary(
    ...
    lines = [
        ...
        *(f"  {k}: {result_stats.get(k, 0)}"
          for k in ["buses", "gens", "loads", "stors", "lines", "icos", "hvdc", "trafos"]),
        "",
        "Run log:",
        *log_lines,
    ]
...
def main():
    ...
    log(
        ...
        f"hvdc={len(section_items(data, 'HVDCLink'))} "
    ...
    reject_empty_keep_after_validation(selectors_r, selectors, keep_codes)
    ...
    if uses_pure_disaggregated:
        ...
        log(
            ...
            f"lines={stats['lines']} icos={stats['icos']} hvdc={stats.get('hvdc', 0)} trafos={stats['trafos']}"
        )
    else:
    ...
    storage_integrity_errors = int(stats.pop("_storage_integrity_errors", 0) or 0)
    dangling_n = int(stats.pop("_dangling_relations", 0) or 0)

    if export_profiles and series_dict:
        T = pm.T if pm is not None else 8760
        out_ts_id = output_timestamp_series_id(out_obj)
        if _out_format == "parquet":
            pq_out = outdir / "cesdm" / "profiles" / "profiles.parquet"
            write_profiles_parquet(pq_out, series_dict, T, ts_id=out_ts_id)
            log(f"wrote profiles (parquet)={pq_out} n_series={len(series_dict)}")
        else:
            write_profiles_h5_cesdm(h5_out_path, series_dict, T, ts_id=out_ts_id)
            log(f"wrote profiles (hdf5)={h5_out_path} n_series={len(series_dict)}")

    if export_yaml:
        out_yaml = outdir / "cesdm" / "yaml" / f"aggregated_cesdm_{level}.yaml"
        out_model = data_to_model(schemas_dir, out_obj, log=log)
    ...
    if storage_integrity_errors or dangling_n:
        raise SystemExit(
            f"Aggregation finished with {storage_integrity_errors} storage-integrity "
            f"warning(s) and {dangling_n} dangling relation(s); see the log."
        )
