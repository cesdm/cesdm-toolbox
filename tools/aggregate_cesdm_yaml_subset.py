...
ReservoirStorageUnit, TransmissionLine, Interconnector, HVDCLink,
...
layer and no "representsAsset" indirection.
...
# ".PowerFlowView" pseudo-class with a "representsAsset" relation back to
...
# ReservoirStorageUnit is a standalone class (not a StorageUnit subclass --
# see schemas/cesdm/.../ReservoirStorageUnit.yaml), so the two need separate
...
STORAGE_RESERVOIR_CLASS = "ReservoirStorageUnit"
STORAGE_ASSET_CLASSES = [STORAGE_NONRESERVOIR_CLASS, STORAGE_RESERVOIR_CLASS]

BRANCH_ASSET_CLASSES = ["TransmissionLine", "Interconnector", "HVDCLink", "Transformer"]
...
def is_phs_paired_generator(data: Dict[str, Any], gen_asset_id: str) -> bool:
    ...
    is_reversible attribute, its own machine_role/
    ...
    if bool(attr_value(gen_ent, "is_reversible")):
        return True
    if str(attr_value(gen_ent, "machine_role") or "").lower() == "reversible":
        return True
...
def is_phs_reservoir(data: Dict[str, Any], reservoir_id: str) -> bool:
    """True if this ReservoirStorageUnit is fed by at least one
    ...
    aggregation groups: both aggregated generators' drawsFromReservoir
    ...
    for gen_id, gen_ent in section_items(data, "HydroGenerationUnit").items():
        if first_rel(gen_ent, "drawsFromReservoir") == reservoir_id:
...
def aggregated_storage_id_for_asset(
    ...
    for asset_class, members in sto_by_class.items():
        ...
        if ent is None:
            continue
        tech = attr_value(ent, "storage_technology_type") or asset_technology_tag(data, asset_class, asset_id, asset_class)
...
def generation_profile_relation_for_bucket(bucket: str) -> str:
    return "hasRunOfRiverInflowProfile" if bucket == "hydro" else "hasAvailabilityProfile"
...
# TopologyView entity or representsAsset indirection exists in the current
...
def model_to_data(model: "CesdmModel") -> Dict[str, Any]:
    ...
    no "representsAsset" indirection to resolve. This is a direct,
...
def data_to_model(schemas_dir: Path, data: Dict[str, Any]) -> "CesdmModel":
    ...
    "representsAsset" indirection to resolve first.
    """
    model = build_model_from_yaml(str(schemas_dir))
...
def build_subset_disaggregated(
    ...
    kept_reservoir_ids = {
        first_rel(section_items(data, "HydroGenerationUnit")[gid], "drawsFromReservoir")
        for gid in out_gen_assets_by_class.get("HydroGenerationUnit", {})
    } - {None}

    def _keep_storage(cls: str, aid: str) -> bool:
        if cls == "ReservoirStorageUnit":
...
def summarize_by_country(
    ...
    capacity, reservoir capacity and natural inflow, and demand.
...
def aggregate_subset(
    ...
    a2n = build_asset_to_node(data, GENERATION_ASSET_CLASSES + ["DemandUnit"])
    ...
    # hasNaturalInflowProfile, hasRunOfRiverInflowProfile -- whichever it has) ──
    ...
    for asset_class in GENERATION_ASSET_CLASSES:
        ...
        for asset_id, ent in section_items(data, asset_class).items():
            ...
            if bus_id not in node_to_agg:
                continue
            tech = attr_value(ent, "generator_technology_type") or asset_technology_tag(data, asset_class, asset_id, asset_class)
    ...
    for (bucket, agg_bus, tech), members in gen_groups.items():
        ...
        if "machine_role" in allowed:
            roles = [attr_value(e, "machine_role") for _, e in members if attr_value(e, "machine_role")]
            if roles:
                gen_attrs.append(make_attr("machine_role", Counter(roles).most_common(1)[0][0]))
        ...
        if bucket == "hydro":
            ...
            if mapped_reservoirs:
                gen_rels.append(make_rel("drawsFromReservoir", mapped_reservoirs))
        else:
            mapped_reservoirs = []
        ...
        if not has_own_profile:
            ...
            # hasAvailabilityProfile/hasRunOfRiverInflowProfile (e.g.
            ...
            # no hasRunOfRiverInflowProfile to lose. Only a plain
            ...
            if is_nondispatchable:
                if bucket == "hydro" and mapped_reservoirs:
                    ...
                    # directly on the linked ReservoirStorageUnit entity
                    # itself") -- this generator correctly has no
                    # hasRunOfRiverInflowProfile of its own; the real
    ...
    # ── Aggregated storage ─────────────────────────────────────────────────────
    # Non-reservoir StorageUnit and ReservoirStorageUnit are kept as
    # separate output classes (ReservoirStorageUnit is a standalone
    ...
    for asset_class, members_by_id in sto_by_class.items():
        for asset_id, ent in members_by_id.items():
            ...
            # reachable via drawsFromReservoir.
            bus_id = a2n.get(asset_id) or reservoir_bus.get(asset_id)
            if bus_id in node_to_agg:
                tech = attr_value(ent, "storage_technology_type") or asset_technology_tag(data, asset_class, asset_id, asset_class)
                country = bus_country.get(bus_id, "")
                ...
                if asset_class == STORAGE_RESERVOIR_CLASS:
                    ...
                    # drawsFromReservoir both point at one, physically
    ...
    for (asset_class, agg_bus, tech), members in sto_groups.items():
        ...
        if asset_class == STORAGE_NONRESERVOIR_CLASS:
            ...
            # right (unlike a ReservoirStorageUnit).
    ...
    # directly on the linked ReservoirStorageUnit entity itself." Only
    ...
    def _process_branch(
        ...
        link_groups[(branch_type, x, y)].append((asset_id, asset_ent))
    ...
    for aid, ent in section_items(data, "TransmissionLine").items():
        _process_branch(aid, ent, link_groups, "TransmissionLine")
    for aid, ent in section_items(data, "Interconnector").items():
        _process_branch(aid, ent, link_groups, "Interconnector")
    for aid, ent in section_items(data, "HVDCLink").items():
        _process_branch(aid, ent, link_groups, "HVDCLink")
    for aid, ent in section_items(data, "Transformer").items():
        _process_branch(aid, ent, link_groups, "Transformer")
    ...
    for (btype, a, b), members in link_groups.items():
        ...
        if btype == "TransmissionLine":
            ents       = [e for _, e in members]
            caps       = [attr_float(e, "thermal_capacity_rating") for e in ents]
            xs_vals    = [attr_float(e, "series_reactance_per_km")        for e in ents]
            lens       = [attr_float(e, "line_length")             for e in ents]
            cap_sum    = sum(x or 0.0 for x in caps)
            ...
            if lw is not None:
                attrs.append(make_attr("line_length",    float(lw), "km"))
            if xw is not None:
                attrs.append(make_attr("series_reactance_per_km", float(xw), "ohm"))

            out_lines[eid] = {
                "attributes": attrs,
                "relations": [make_rel("fromNode", [a]), make_rel("toNode", [b])],
            }

        elif btype == "Interconnector":
            ents = [e for _, e in members]
            caps12    = [attr_float(e, "maximum_power_flow_from_to") for e in ents]
            cap12_sum = sum(x or 0.0 for x in caps12)
            caps21    = [attr_float(e, "maximum_power_flow_to_from") for e in ents]
            ...
            ents = [e for _, e in members]
            caps = [attr_float(e, "p_max_hvdc") or attr_float(e, "max_flow") for e in ents]
            cap_sum = sum(x or 0.0 for x in caps)
            ...
            out_hvdcs[eid] = {
                "attributes": [
                    ...
                    make_attr("hvdc_technology_type", (lambda vals: vals[0] if vals and all(v == vals[0] for v in vals) else "VSC")([str(attr_value(e, "hvdc_technology_type")) for e in ents if attr_value(e, "hvdc_technology_type") in ("LCC", "VSC")]) ),
            ...
            ents     = [e for _, e in members]
            caps     = [attr_float(e, "thermal_capacity_rating")  for e in ents]
            vprim    = [attr_float(e, "rated_primary_voltage")    for e in ents]
            vsec     = [attr_float(e, "rated_secondary_voltage")  for e in ents]
            scv      = [attr_float(e, "short_circuit_voltage_in_percentage")    for e in ents]
            npar     = [attr_float(e, "parallel_circuit_count")   for e in ents]
            cap_sum  = sum(x or 0.0 for x in caps)
            ...
            if np_sum > 0:
                attrs.append(make_attr("parallel_circuit_count", float(np_sum)))
            if vp is not None:
                attrs.append(make_attr("rated_primary_voltage",  float(vp), "kV"))
            if vs is not None:
                attrs.append(make_attr("rated_secondary_voltage", float(vs), "kV"))
            if scvw is not None:
                attrs.append(make_attr("short_circuit_voltage_in_percentage",   float(scvw), "%"))
    ...
    agg_bus_country = {
        agg_id: country_of_nuts3(agg_id.split(".")[1])
        for agg_id in out_buses
    }
    agg_a2n = build_asset_to_node(out_obj, GENERATION_ASSET_CLASSES + ["DemandUnit"])
