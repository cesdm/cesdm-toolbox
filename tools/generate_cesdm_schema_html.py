...
def categorise(name: str, entities: dict) -> str:
    ...
    Categories:
      ...
      result  — descends from Result (the only classes that use the
                representsAsset pattern, one per analysis run; AVR/GOV/PSS
    ...
    if ancestors & _REPR_ROOTS:
        ...
        # representsAsset target, so they'd never be nested under any
...
def views_for_asset(asset_name: str, entities: dict) -> list[str]:
    """Return all Result entities that have representsAsset → asset_name."""
    result = []
...
def sidebar_nav(entities: dict) -> str:
    ...
    sections = [
        ...
        ("Results", [n for n, e in sorted(entities.items())
                              if e.get("category") == "result"
