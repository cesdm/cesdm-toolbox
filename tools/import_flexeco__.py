...
def _ensure_gen_dispatch(model: CesdmModel, asset_id: str,
    ...
    """Return the generation asset itself; dispatch data is stored directly."""
    return asset_id
...
def build_asset_view_map(model: CesdmModel) -> dict:
    ...
    # Flattened pattern (see CHANGELOG.md): an asset class may have a
    # view's attributes/relations merged directly onto it rather than a
    # separate view entity with its own representsAsset link -- the
    # loop above would then never find or register it at all. For
    # every asset whose class supports at least one belongsToGroup
    # value, register the asset *itself* under every concrete view
    # class name that shares that same group, so a caller doing
    # avm.get(asset_id, {}).get("ReservoirStorageUnit.DispatchView")
    # (or any other specific class name) finds the asset's own data
    # instead of silently getting None. Found directly, from a real
    # PHS scenario built with add_phs_closed_loop() and exported via
    # export_to_flexeco() without going through the aggregation tool's
    # own synthesis first: the reservoir was skipped as "no inflow
    # data" and the generator misclassified as PN_GenDispatchable
    # instead of PN_StoragePumpNoInfeed, exactly the kind of
    # misclassification bug fixed earlier this session for the
    # aggregation-tool path specifically.
    #
    # This is a hardcoded, local mapping rather than a schema lookup
    # (checking each class's view_family) -- the old representation-view
    # schema classes that mapping used to introspect no longer exist at
    # all (this toolbox's "initial version without views"). Only the
    # exact class names checked throughout this file matter here.
    _CANONICAL_NAMES_BY_GROUP: dict = {
        "dispatch": {
            "HydroGenerationUnit.DispatchView", "Generation.DispatchView",
            "ReservoirStorageUnit.DispatchView", "Storage.DispatchView",
            "Demand.DispatchView", "ExternalSupply.DispatchView", "HVDCLink.DispatchView",
        },
        "power_flow": {
            "Generator.PowerFlowView", "Demand.PowerFlowView", "ElectricalBus.PowerFlowView",
            "TransmissionLine.PowerFlowView", "HVDCLink.PowerFlowView",
            "Interconnector.PowerFlowView", "Transformer.PowerFlowView", "Shunt.PowerFlowView",
        },
        "topology": {"SinglePort.TopologyView", "TwoPort.TopologyView"},
    }
    ...
    for asset_cls_name, entities in model.entities.items():
        ...
        if cdef is None or getattr(cdef, "abstract", False):
            continue
        groups_on_asset: set = set()
        for d in (getattr(cdef, "attributes", {}) or {}).values():
            groups_on_asset.update(getattr(d, "belongsToGroup", None) or [])
        for d in (getattr(cdef, "relations", {}) or {}).values():
            groups_on_asset.update(getattr(d, "belongsToGroup", None) or [])
        if not groups_on_asset:
            continue
        view_class_names: set = set()
        for group in groups_on_asset:
            view_class_names.update(_CANONICAL_NAMES_BY_GROUP.get(group, ()))
        if not view_class_names:
            continue
        for asset_id, ent in entities.items():
            slot = asset_view_map.setdefault(asset_id, {})
            for vcls in view_class_names:
                slot.setdefault(vcls, ent)
...
def export_to_flexeco(model: CesdmModel, output_path: str | Path, *, hdf5_path: str | Path | None = None) -> None:
    ...
    for sid, ent in stor_ents.items():
        ...
        # reservoir has no SinglePort.TopologyView of its own.
        ...
        if bus_id is None:
            for _gid, _gent in (model.entities.get("HydroGenerationUnit") or {}).items():
                ...
                if sid in _gdraws_list:
                    bus_id = _bus_from_nodal_view(_gid)
        ...
        def _sv(name, default=None):
            ...
            1. Storage/Reservoir DispatchView
            2. StorageUnit or ReservoirStorageUnit asset itself
            3. Referenced StorageType
            ...
            be stored directly on the asset rather than on a separate view
            entity.
            """
            val = _av(sv, name)
        ...
        if capacity is None:
            print(
                ...
                "its dispatch view, asset entity, or StorageType; "
