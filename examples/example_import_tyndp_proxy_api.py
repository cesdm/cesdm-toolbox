...
def _assign_generation_row(model: CesdmModel, tech_id: str, type_name: str,
    ...
    if model.has_entity(tech_id):
        ...
        if generation_class == "HydroGenerationUnit":
            ...
            # machine_role is always "turbine" here.
...
def _assign_storage_row(model: CesdmModel, tech_id: str, type_name: str,
    ...
    if model.has_entity(tech_id):
        ...
        model.add_entity(entity_class="ReservoirStorageUnit", entity_id=tech_id)
        ...
        # below -- removed: ReservoirStorageUnit is a standalone class now
    ...
    if is_phs_type:
        ...
        if model.has_entity(gen_id):
            ...
            model.add_relation(gen_id, "atNode", bus_id)
            model.add_relation(gen_id, "drawsFromReservoir", tech_id)
            model.add_relation_if_allowed(tech_id, "suppliesResourceTo", gen_id)
            gen = model.get_entity(entity_id=gen_id)
            gen.dispatch.machine_role = "turbine"
            gen.name = gen_id
        if cap_mw is not None:
...
def assign_nodes_and_countries_from_tyndp_nodes_csv(
    ...
    for _, row in df.iterrows():
        ...
        if not model.has_entity(country_id):
            model.ensure_entity(class_name="GeographicalRegion", entity_id=country_id, name=cc_name)
            created_countries += 1
        else:
...
def assign_ntc_from_tyndp_ntc_types_base_csv(
    ...
    *,
    ...
    for _, row in df_final.iterrows():
        ...
        model.ensure_entity(class_name="Interconnector", entity_id=ntc_id, name=f"NTC {frm}->{to}")
