...
For convenience, both datasets are bundled in:
...
Already converted CESDM packages (YAML, Frictionless, HDF5 profiles) for the PyPSA nodal example and the published TYNDP scenarios:

```bash
cesdm-download-converted-data
```

That extracts under `converted_data/PYPSA/` and `converted_data/TYNDP2024/`, matching `external_data/`.

Manual alternative:

> https://ethz.ch/content/dam/ethz/special-interest/mavt/ctr-energy-networks-fen-dam/data/cesdm_converted_data.zip
...
```bash
python ./examples/example_import_pypsa.py --nc-path ./external_data/PYPSA/elec.nc --schema-dir schemas/cesdm --nuts-shapefile external_data/PYPSA/NUTS_RG_20M_2021_4326.shp --output-dir ./output/PYPSA/nodal/
```
---
