...
For convenience, the required **TYNDP 2024** datasets together with the original **PyPSA** example network (`elec.nc`) are bundled in:
...
Already converted CESDM packages (YAML, Frictionless, HDF5 profiles) for the published TYNDP scenarios and the PyPSA nodal example:
