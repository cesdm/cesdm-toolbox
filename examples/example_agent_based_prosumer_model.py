...
def _ensure_base_system(model):
    ...
    model.ensure_entity(class_name="GeographicalRegion", entity_id="region.ch", name="Switzerland")
    ...
    model.add_relation("bus.lv.demo", "locatedIn", "region.ch")
    model.add_attribute("bus.lv.demo", "powerflow_bus_type", "PQ")
...
def build_agent_based_prosumer_model(input_yaml: Path | None = None):
    ...
    for i, hh in enumerate(households, start=1):
        ...
        if hh["pv_kw"] > 0:
            ...
            model.add_relation(pv_id, "hasTechnology", "Generation.Renewable.Solar.PV")
