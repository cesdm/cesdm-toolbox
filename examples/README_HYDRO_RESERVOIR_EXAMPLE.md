...
```python
m.add_entity("ReservoirStorageUnit", "reservoir.alpine")
...
The `ReservoirStorageUnit` and the `HydroGenerationUnit` are two
separate entities, linked by the `drawsFromReservoir`/
...
```python
m.add_entity("ReservoirStorageUnit", "reservoir.grimsel.upper")
m.add_entity("ReservoirStorageUnit", "reservoir.grimsel.lower")
...
reservoir, and the reversible turbine — with `drawsFromReservoir`/
`suppliesResourceTo`/`dischargesToReservoir` all wired between them
explicitly. The turbine can both generate (upper → lower, producing
electricity) and pump (lower → upper, consuming it) — that's what
`is_reversible=True` and `maximum_pumping_power` describe.
