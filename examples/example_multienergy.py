...
def build_multienergy_model(schema_dir: Path) -> CesdmModel:
    ...
    m.ensure_entity(class_name="GeographicalRegion", entity_id="CH", name="Switzerland")
    ...
    m.add_relation_if_allowed(n_elec, "locatedIn", "CH")
    ...
    #   onto the asset) only declare dispatch participation and the
    #   reference port -- the port entities carry the conversion
    #   semantics.
    # ------------------------------------------------------------------
    m.add_entity(entity_class="ConversionUnit", entity_id="CHP_1")
