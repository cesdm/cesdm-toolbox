...
def print_summary(model):
    ...
    for gen_id in _GEN_BUSES:
        ...
        # .power_flow resolves the generator's PowerFlowView through the
        # proxy API rather than reconstructing its id manually -- this
        # remains correct regardless of the underlying auto-generated id.
        pm = model.get_entity(entity_id=gen_id).power_flow.active_power_setpoint
    ...
    print(f"\n  (Reactances in pu on machine base Sn={Sn_mva} MVA, Vn={Vn_kv} kV  —  "
          f"xd={_MACHINE_PU['d_axis_synchronous_reactance']} pu, "
          f"xd'={_MACHINE_PU['d_axis_transient_reactance']} pu, "
          f"xd''={_MACHINE_PU['d_axis_subtransient_reactance']} pu)")

    print("\n  Line impedances (converted from pu on system base)")
    for line_id, fb, tb, r_pu, x_pu, b_pu in _LINES:
        # Same reasoning as above -- .power_flow instead of reconstructing
