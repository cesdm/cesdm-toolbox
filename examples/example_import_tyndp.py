...
def _storage_asset_class_for_type(type_name: str, type_id: str | None = None) -> str:
    ...
    ReservoirStorageUnit: reservoir hydro, pondage, AND all PHS types.
      ...
      (has_active_charging, annual_natural_inflow_energy).
...
def _add_hydro_reservoir_composite_if_missing(model: CesdmModel, reservoir_id: str, bus_id: str | None,
    ...
    model.add_attribute(gen_id, "machine_role", hydro_machine_role(hydro_tt_id, is_reversible=is_reversible))
...
def assign_nodes_and_countries_from_tyndp_nodes_csv(
    ...
    for _, row in df.iterrows():
        ...
        if country_id not in model.entities.get("GeographicalRegion", {}):
            model.add_entity(entity_class="GeographicalRegion", entity_id=country_id)
            created_countries += 1
        else:
            updated_countries += 1
    ...
    *,
    ...
    for _, row in df_agg.iterrows():
        ...
        if cid not in model.entities.get("GeographicalRegion", {}):
            model.add_entity(entity_class="GeographicalRegion", entity_id=cid)
        ...
        if is_stor:
            ...
            if not _entity_exists(model, tech_id):
                ...
                if is_hydro_res:
                    ...
                    # No atNode here: ReservoirStorageUnit is a standalone
            ...
            # ReservoirStorageUnit and StorageUnit both hold their dispatch
...
def assign_inflow_timeseries_from_csv(
    ...
    *,
    ...
    annual_natural_inflow_energy / natural_inflow_profile_reference
      → held directly on the StorageUnit/ReservoirStorageUnit itself
    profile dict key → Profile entity (V4 new)
    """
    ...
    for _, row in df_agg.iterrows():
        ...
        if asset_cls in {"GenerationUnit", "HydroGenerationUnit"}:
            # Run-of-river and other generation inflows: resource potential
            # held directly on the asset itself (see CHANGELOG.md).
            gdv = tech_id
            model.add_attribute(gdv, "annual_resource_potential",
                                {"value": annual})
    ...
    *,
    ...
    for _, row in df_final.iterrows():
        ...
        if not _entity_exists(model, ntc_id):
            model.add_entity(entity_class="Interconnector", entity_id=ntc_id)
...
def prune_hydro_reservoirs_without_inflow(model: CesdmModel) -> dict:
    ...
    for reservoir_id in list((model.entities.get("ReservoirStorageUnit") or {}).keys()):
        ...
        if not any(x in key for x in ("reservoir", "pondage")):
            continue
        # ReservoirStorageUnit holds annual_natural_inflow_energy directly
        # (see CHANGELOG.md).
